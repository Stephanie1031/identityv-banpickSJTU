...
更了更了，这个bug终于被我修了，现在仅需提前导入队徽，其他部件都可以在生成窗口后实时更新同步。
...
2024/7/19 更新
> 加入了新求生和监管（虽然只是修改了文本文档）

> 一时兴起修了一堆bug，总之应该更好用了

> 建议自行替换背景图片来满足不同使用需要，也可以使用蓝底在obs中抠图替换

> 其实比分悬浮窗也做好了，但感觉没什么必要加入（如有需要可以单独出一个新版本）

> 生成了单文件exe和文件夹exe两个版本，可按需取用
